...
def train(args, model, optimizer, train_loader, epoch):
	...
	for iteration, batch in enumerate(train_loader, 1):
		...
		[ori, mask, fg, bg, trimap] = batch_new
		optimizer.zero_grad()
		predict_global, predict_local, predict_fusion, predict_global_side2, predict_global_side1, predict_global_side0 = model(ori)
		...
		if iteration !=  0:
			...
			args.logging.info("P3M-NET-Epoch[{}/{}]({}/{}) Lr:{:.8f} Loss:{:.5f} Global:{:.5f} Local:{:.5f} Fusion-alpha:{:.5f} Fusion-comp:{:.5f} Speed:{:.5f}s/iter {}".format(epoch, args.nEpochs, iteration, num_iter, optimizer.param_groups[0]['lr'], loss.item(), loss_global.item(), loss_local.item(), loss_fusion_alpha.item(), loss_fusion_comp.item(),speed, exp_time))
